...
class RobotVisualization:
    def __init__(self, num_robots, width, height, obstacle_tiles, delay = 0.2):
    ...
    def _status_string(self, time, num_clean_tiles, num_total_tiles):
        ...
        return "Time: %04d; %d tiles (%d%%) Scanned" % \
    ...
    def update(self, room, robots):
        ...
        for i in range(self.width):
            for j in range(self.height):
                ...
                elif self.obstacle_tiles and room.is_tile_obstacle(i, j):
        ...
        for robot in robots:
            ...
            self.robots.append(self.w.create_oval(x1, y1, x2, y2,
                                                  fill = "black"))
            self.robots.append(
                self._draw_robot(robot.get_drone_position(), robot.get_drone_direction()))
        # Update text
        self.w.delete(self.text)
        self.time += 1
        self.text = self.w.create_text(
            25, 0, anchor=NW,            
            text=self._status_string(self.time, room.get_num_explored_tiles(), room.get_num_tiles()))
        self.master.update()
